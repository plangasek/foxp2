...
```{r celf}
elvs %>% ggplot(aes(x=CELF7SS0, fill = "red")) + geom_density() + geom_vline(xintercept = 100) + ggtitle('CELF7SS0 Language Scores')
...
```{r}
elvs %>% ggplot(aes(x=rs1916988,fill=rs1916988)) + geom_bar() + ggtitle('Genotype Freqeuncy for rs1916988 in ELVS')
...
ggsave('images/CELF7SS0_scores_dist.png')
```
...
```{r}
iowa %>% ggplot(aes(x=rs1916988,fill = rs1916988)) + 
  geom_bar(position = "stack") +
  ggtitle('Iowa Genotype Variation Counts')
...
common_SNP %>% 
  ggplot(aes(x = LCOMP, color = rs1916988, alpha = 0.01)) + geom_density() + facet_wrap(~Sample) + ggtitle('Iowa Score for rs1916988 by sample')
